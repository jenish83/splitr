 ...
 *   owed:   { to: string;   amount: number }[];  // this member → others
...
export function GroupBalances({ balances }) {
  ...
  /* ───── helpers ───────────────────────────────────────────────────────── */
  const me = balances.find((b) => b.userId === currentUser._id);
  if (!me) {
    return (
      <div className="text-center py-4 text-muted-foreground">
  ...
  const userMap = Object.fromEntries(balances.map((b) => [b.userId, b]));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-center pb-4 border-b">
        ...
        >
          {format(me.totalBalance, { signed: true })}
        </p>
